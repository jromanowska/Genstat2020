...
```{r}
library(Haplin)
# temporarily, read in updated functions from the script;
#   these will be published on CRAN shortly
...
We will re-do the QC from DAY 1 on the entire dataset (not only children) and based on that, we will choose a subset of the data.

```{r}
library(snpStats)

data_dir <- file.path("..", "..", "DAY1", "genetics_R")
raw.all <- read.pedfile(file = file.path(data_dir, "data.ped"),
                        snps = file.path(data_dir, "data.map"), which = 2)
raw.all.gen <- raw.all$genotypes

qc.all <- row.summary(raw.all.gen)
summary(qc.all)

ok.call.rate <- qc.all$Call.rate >= 0.95
raw.all.ok <- list(genotypes = raw.all$genotypes[ok.call.rate,],
                      fam = raw.all$fam[ok.call.rate,],
                      map = raw.all$map)

qc.all.marker <- col.summary(raw.all.ok$genotypes)
summary(qc.all.marker)

ok.MAF <- qc.all.marker$MAF >= 0.05
raw.all.ok <- list(genotypes = raw.all.ok$genotypes[,ok.MAF],
                      fam = raw.all.ok$fam,
                      map = raw.all.ok$map[ok.MAF,])

qc.all.marker2 <- col.summary( raw.all.ok$genotypes )
ok.markers <- qc.all.marker2$Call.rate >= 0.99
```

Choosing the subset

```{r}
raw_all <- genDataRead(file.in = file.path(data_dir, "data.ped"), format = "ped",
                       map.file = file.path(data_dir, "data1.map"))
raw_all
qc_all <- genDataGetPart(raw_all, design = "cc", rows = which(ok.call.rate),
                         markers = which(ok.MAF), file.out = "qc_all_ok1")
qc_all <- genDataGetPart(qc_all, design = "cc", markers = which(ok.markers),
                         file.out = "qc_all_ok")
qc_all
...
```{r}
orig.map <- read_delim(file.path(data_dir, "data1.map"), delim = " ")
orig.map
new.map <- orig.map %>%
  filter(snpname %in% showSNPnames(qc_all_triads, n = "all"))
new.map
write_delim(new.map, path = "qc_all.map", delim = " ", col_names = TRUE)
```
...
```{r}
my.prepared.ped.data <- genDataPreprocess(data.in = qc_all_triads,
  map.file = "qc_all.map", design = "triad", file.out = "qc_all_preproc")
my.prepared.ped.data
showSNPnames(my.prepared.ped.data)
```
