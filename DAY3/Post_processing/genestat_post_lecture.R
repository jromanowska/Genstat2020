...
## Set working directory to the subdirectory where you can save the files from 
##  this R intoduction course, e.g., C:\genestat\R_intro
  ## Write setwd("directory name"). E.g., setwd("C:\genestat\R_intro")
  ## Write load("genestat.RData")

## Packages have already been installed
## install.packages(c("GenABEL","Haplin"))
...
## Save workspace. This ensures that .First will be ran when genestat_post.RData
##  is opened (double click on icon).
save.image("genestat_post.RData")



############################################
## Full trio dataset from first R-session ##
############################################
## Look at data
str(raw.all)
...
##############################################################################
## p.level = 0 sets the rejection level for a HWE-test.
##  Because we have trio data, this will always fail. 
##############################################################################

## maf = 0.05 sets the lowest permitted minor allele frequency (Haplin uses 5%)
## Other parameters have default values in check.marker (see help file)
## Step 3.
## Repeat Step 2 until no more markers and individuals are removed
qc <- check.marker(raw.all, extr.call = 0.1, extr.perid.call = 0.1, 
                            callrate = 0.99, perid.call = 0.95, 
                            p.level=0, maf=0.05) 

## SNPs that are OK
length(qc$snpok)
head(qc$snpok)
## Individuals that are OK
length(qc$idok)
head(qc$idok)

## Data set after quality control
qc.all <- raw.all[qc$idok,qc$snpok]
str(qc.all)

## Run haplin
haplinRuns <- haplinSlide(data=qc.all,pedIndex="data.pedIndex",
                   winlength=1,response="mult",reference="ref.cat")
## Class of result
class(haplinRuns)
