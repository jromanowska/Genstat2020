...
```{r}
haplin.data <- read.table("HAPLIN.trialdata.txt", stringsAsFactors = FALSE)
head(haplin.data)
```
...
my.ped.data <- genDataRead(file.in = "data.ped", file.out = "my_ped_data",
  dir.out = ".", format = "ped")
my.haplin.data <- genDataRead(file.in = "HAPLIN.trialdata.txt",
...
```{r}
my.haplin.data2 <- genDataRead(file.in = "HAPLIN.trialdata.txt",
  file.out = "my_haplin_data_cov", cov.file.in = "add_cov.dat",
...
```{r}
my.prepared.ped.data <- genDataPreprocess(data.in = my.ped.data, map.file = "data1.map",
...
head(my.prepared.ped.data$cov.data)
my.prepared.ped.data$gen.data[[ 1 ]][ 1:10,1:10 ]
```
