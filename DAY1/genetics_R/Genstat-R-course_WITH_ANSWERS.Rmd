...
```{r ex-1-2}
round(table(snp8)/nrow(snp8), 2)
sum(round(table(snp8)/nrow(snp8), 2))
```
...
```{r ex-3-1}
obj2 <- read_table2("data_c_intro_R.dat")
obj2
...
```{r}
chr1.snps <- as.character(
    raw.all$map$snp.names[raw.all$map$chromosome == "1"]
  )
chr1.snps
```
...
```{r ex-5-3}
raw.mothers.new <- raw.mothers$genotypes[,!colnames(raw.mothers$genotypes) %in% c("rs12", "rs90")]
dim(raw.mothers.new)
```
